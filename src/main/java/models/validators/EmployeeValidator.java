...
public class EmployeeValidator {
     ...
     * @param codeDupilicateCheckFlag 社員番号の重複チェックを実施するかどうか(実施する:true 実施しない: false)
    ...
    public static List<String> validate(
        ...
        // パスワードのチェック
        String passError = validatePassward(ev.getPassword(), passwordCheckFlag);
        if (!passError.equals("")) {
     ...
     * @param passwardCheckFlag パスワードの入力チェックを実施するかどうか(実施する:true 実施しない:false)
     * @return エラーメッセージ
     */
    private static String validatePassward(String passward, Boolean passwardCheckFlag) {

        // 入力チェックを実施 かつ 入力値がなければエラーメッセージを返却
        if (passwardCheckFlag && (passward == null || passward.equals(""))) {
